...
class MysqlContainer:
    def __init__(self, local_yml_path, service_name=None, remote_host=None, remote_user=None, remote_password=None,
        ...
        """
        初始化 CreateContainer 类的实例。

        参数:
            remote_host (str): 远程主机的 IP 地址或主机名。
            remote_user (str): 远程主机的用户名。
            remote_password (str): 远程主机的密码。
            db_type (str): 数据库类型，'remote' 表示远程数据库，'local' 表示本地数据库。
            local_yml_path (str): 本地 docker-compose.yml 文件路径。
            remote_yml_dir (str): 远程目标路径，用于存放上传的 docker-compose.yml 文件。
            service_name (str or list): 数据库服务名称，可以是单个服务名称或服务名称列表。
            max_attempts (int): 检查数据库就绪的最大尝试次数，默认为 10。
            sleep_time (int): 每次尝试之间的休眠时间（秒），默认为 5。
        """
    ...
    def start_database_remote(self, name):
        ...
        # 切换到包含 docker-compose.yml 文件的目录并启动数据库
...
if __name__ == "__main__":
    ...
    # 远程目标路径，用于存放上传的 yml 文件
    remote_yml_dir = "/home/zym/test1"

    # 单个服务启动（本地）
    service_name_single_local = "service1"
    db_local_single = MysqlContainer(local_yml_path=local_yml_path, service_name=service_name_single_local)
    db_local_single.start_databases()

    # 单个服务启动（远程）
    db_remote_single =  MysqlContainer(local_yml_path=local_yml_path, service_name=service_name_single_local,
    ...
    # 批量启动（本地）
    service_names_batch_local = ["service1", "service2", "service3"]  # 传入服务名称列表
    db_local_batch =  MysqlContainer(local_yml_path=local_yml_path, service_name=service_names_batch_local)
    db_local_batch.start_databases()

    # 批量启动（远程）
    db_remote_batch =  MysqlContainer(local_yml_path=local_yml_path, service_name=service_names_batch_local,
                                       remote_host=remote_host, remote_user=remote_user, remote_password=remote_password,
                                       db_type='remote', remote_yml_dir=remote_yml_dir)
    db_remote_batch.start_databases()

    # 全部启动（本地）
    db_local_all =  MysqlContainer(local_yml_path=local_yml_path)
    db_local_all.start_databases()

    # 全部启动（远程）
    db_remote_all =  MysqlContainer(local_yml_path=local_yml_path, remote_host=remote_host, remote_user=remote_user,
