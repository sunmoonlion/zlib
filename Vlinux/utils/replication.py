...
class MySQLReplication:
    ...
    def get_database(self, user, password, host, port):
        if self.container_exist:
            container = Container(self.local_path, self.remote_path, self.service_name, 
                                self.remote_host, self.remote_user, self.remote_password,
                                self.location_type, self.max_attempts, self.sleep_time, 
                                self.private_key_path,self.location_type)
            try:
...
if __name__ == "__main__":
    user_master = "zym"
    password_master="alyfwqok"
    user_slave = "zym"
    password_slave="alyfwqok"
    local_path = "/home/zym/docker-compose-files"
    remote_path = "/home/zym/docker-compose-files"
